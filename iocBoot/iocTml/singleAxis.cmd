#
# singleAxis.cmd — Example: single Technosoft axis
#
...
#   devicePath — OPTION A: "IP:port"     direct TCP (Moxa NPort, XPORT, ser2net)
#              — OPTION B: /dev/ttyUSBx  local serial RS-232/RS-485
#              — OPTION C: /var/tmp/ttyV0  socat PTY bridge (see tools/start_sim.sh)
#   numAxes    — total axes on this channel
#   hostId     — RS-232/485 host ID; for TCP usually the axis ID of the direct drive
#   baudRate   — 9600 (default) or 115200; ignored for TCP connections
#   movingPoll — polling period (s) while moving
#   idlePoll   — polling period (s) while idle

# Option A: TCP connection (Moxa NPort, Digi XPORT, socat ser2net, etc.)
TmlControllerConfig("TML1", "192.168.190.55:4001", 1, 15, 9600, 0.1, 1.0)

# Option B: local serial
#TmlControllerConfig("TML1", "/dev/ttyUSB0",   1, 15, 9600, 0.1, 1.0)

# Option C: socat PTY bridge
